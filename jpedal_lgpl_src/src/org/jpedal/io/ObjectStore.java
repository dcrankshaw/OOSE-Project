...
public static final String multiThreaded_root_dir=null;
	...
	private final Map image_type = new Hashtable();
    ...
    public static final Integer IMAGE_MASKCOL=new Integer(5);

    /**paramter stored on cached images*/
    public static final Integer IMAGE_COLORSPACE=new Integer(6);
       ...
       // }
		...
		if (type.indexOf("tif") != -1) {
			...
			if (((type_id == 1) | (type_id == 2))&&(current_image.indexOf("HIRES_")==-1))
				...
				/* <end-me>
				image = ColorSpaceConvertor.convertColorspace(image, BufferedImage.TYPE_INT_BGR);
				/**/
...
*/
	 ...
	 */
	private synchronized boolean saveStoredJPEGImage(
		String current_image,
		BufferedImage image,
		boolean file_name_is_path,
		boolean save_unclipped) {
...
//				out.close();
    ...
    public void finalize(){
         ...
         */
        Iterator filesTodelete = undeletedFiles.keySet().iterator();
        while(filesTodelete.hasNext()) {
            String file = ((String)filesTodelete.next());
	...
	public static DynamicVectorRenderer getCachedPage(Integer key) {
		...
		if(cachedFile!=null){
			...
			try {
				...
				from.read(data);
				from.close();
				currentDisplay=new ScreenDisplay(data, new HashMap());
    ...
    public void saveRawImageData(String pageImgCount, byte[] bytes,int w,int h,int pX,int pY, byte[] maskCol, int colorSpaceID) {
        ...
        try {
            ...
            imagesOnDiskColSpaceID.put(key,new Integer(colorSpaceID));
