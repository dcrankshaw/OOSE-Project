...
//					}
    ...
    public static BufferedImage decode(PdfObjectReader currentPdfFile, PdfObject XObject, int subtype){
    	return decode(currentPdfFile, XObject, subtype,0,0,0,0);
    }
     ...
     */
    public static BufferedImage decode(PdfObjectReader currentPdfFile, PdfObject XObject, int subtype,
    		int width, int height,int offsetImage,int iconRotation){
    	...
    	try{
            ...
            PdfStreamDecoder glyphDecoder=new PdfStreamDecoder(currentPdfFile,useHires); //switch to hires as well
    		...
    		if(BBox!=null){
    			...
    			rectX1 = (BBox[0]);
    			rectY1 = (BBox[1]);
				int boxWidth=(int) ((BBox[2]-BBox[0]));
				if(boxWidth<0)
					boxWidth = -boxWidth;
				
				int boxHeight=(int) ((BBox[3]-BBox[1]));
				if(boxHeight<0)
					boxHeight = -boxHeight;
...
//				System.out.println("affine="+affineTransform);
    ...
    public static BufferedImage rotate(BufferedImage src, int rotation) {
        ...
        try{
            ...
            /* <end-me>
            //has been checked on javaSE and works same as previous.
