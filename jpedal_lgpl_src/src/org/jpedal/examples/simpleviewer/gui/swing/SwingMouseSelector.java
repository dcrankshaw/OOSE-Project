				...
				if(id==-1){
					if(clickCount>1){
						...
						case 2 : //double click selects line
							Rectangle[] lines = decode_pdf.getLineAreas(pageMouseIsOver);
							...
							if(lines!=null) { //Null is page has no lines
								for(int i=0; i!=lines.length; i++){
									if(lines[i].intersects(point)){
										...
										decode_pdf.addHighlights(new Rectangle[]{lines[i]}, false, pageMouseIsOver);
						...
						case 3 : //triple click selects paragraph
							Rectangle para = decode_pdf.setFoundParagraph(mouseX,mouseY, pageMouseIsOver);
							if(para!=null){
...
//							co_ords = decode_pdf.getGroupingObject().findTextInRectangle(t_x1,t_y1,t_x2,t_y2,commonValues.getCurrentPage(),textToFind,searchType);
			...
			if(!decode_pdf.isExtractingAsImage()){
				...
				if((altIsDown &&
					...
					//Highlight all within the rectangle
						decode_pdf.addHighlights(new Rectangle[]{currentRectangle}, true, pageOfHighlight);
					...
					decode_pdf.addHighlights(new Rectangle[]{r}, false, pageOfHighlight);
	...
	private Point getPageCoordsInContinuousDisplayMode(int x, int y, int page){
		...
		if (currentGUI.useNewLayout) {
			...
			Rectangle pageArea = new Rectangle(decode_pdf.getPageOffsets(page).x+xAdjustment,
