	...
	public Object executeCommand(int ID, Object[] args) {
		...
		case HIGHLIGHT:
			...
			if(args!=null){
				...
				int page = ((Integer)args[1]).intValue();
				decode_pdf.clearHighlights();
				if(highlights.length==1 && highlights[0]==null){
					//decode_pdf.clearHighlights();
				}else{
					//add text highlight                    
		            decode_pdf.addHighlights(new Rectangle[]{highlights[0]}, true, page);
				}
                //highlights[0].x=1;
...
//                decode_pdf.scrollRectToHighlight(highlights[0]);
		...
		case SCROLL:
			if(args==null){
				...
				if(args.length>1 && args[1]!=null){
					page = ((Integer)args[1]).intValue();
				}
				
				if (scrollTo != null) {
					decode_pdf.scrollRectToHighlight(scrollTo, page);
					decode_pdf.invalidate();
					decode_pdf.repaint();
				}
			}
		...
		case FIND:
			if(args==null){
				...
				if (args.length > 1) {
                    searchType = ((Integer)args[1]).intValue();
                    multiTerms = ((Boolean)args[2]).booleanValue();
                    singlePageSearch = ((Boolean)args[3]).booleanValue();
                } else {
...
//			}
		...
		case PREFERENCES:
            ...
            currentGUI.showMessageDialog("OS version does not support preferences");
            /**/
			break;
			// <start-me>
		case COPY:
		...
		case SELECTALL:
			...
			if(decode_pdf.getDisplayView()==Display.SINGLE_PAGE){
				...
				Rectangle[] allHighlights = decode_pdf.getLineAreas(commonValues.getCurrentPage());
				...
				decode_pdf.addHighlights(allHighlights, true, commonValues.getCurrentPage());
		...
		case MOUSEMODE:
			if(args == null){
                if(decode_pdf.getMouseMode()==PdfDecoder.MOUSE_MODE_TEXT_SELECT){
					
                	//Set Mouse Mode
					decode_pdf.setMouseMode(PdfDecoder.MOUSE_MODE_PANNING);
				...
				}else if(decode_pdf.getMouseMode()==PdfDecoder.MOUSE_MODE_PANNING){
        ...
        case SEPARATECOVER:
            if (args == null){
                //swap option
                decode_pdf.separateCover = !decode_pdf.separateCover;
...
//				Object currPage = results.getTextPages().get(new Integer(index));
				Object currPage = new Integer(commonValues.getCurrentPage());
							...
							if(((Integer) currPage).intValue() != nextPage)
								if(allHighlightsShown){
									...
									for(int k=0; k!=results.getModel().getSize(); k++){
										...
										kInteger=new Integer(k);
										//int currentPage = ((Integer)newPage).intValue();
										if(((Integer)results.getTextPages().get(kInteger)).intValue()==nextPage){
									...
									decode_pdf.addHighlights(finalHighlight, true,nextPage);
								}
...
//							}
							...
							if(!allHighlightsShown)
								decode_pdf.addHighlights(new Rectangle[]{highlight}, true, nextPage);
				...
				if (1==1 || !commonValues.isProcessing()) {// {if(!event.getValueIsAdjusting()){
					...
					if (id != -1) {
						...
						if (newPage != null) {
							...
							if(currPage!=null && ((Integer) currPage).intValue() != nextPage)
								if(allHighlightsShown){
									Vector_Rectangle storageVector = new Vector_Rectangle();
...
//					forward(commonValues.getPageCount() - commonValues.getCurrentPage());
		...
		case FACING:
            ...
            if(args==null){
				if (decode_pdf.turnoverOn && decode_pdf.getPdfPageData().hasMultipleSizes())
                ...
                int p = commonValues.getCurrentPage();
                if(decode_pdf.separateCover && ((p & 1)==1 && p!=1))
                    p--;
                else if (!decode_pdf.separateCover && ((p & 1)==0))
                    p--;
                commonValues.setCurrentPage(p);
                currentGUI.setPage(p);
                currentGUI.hideRedundentNavButtons();
...
//					get values from user
		...
		case BACK:
			decode_pdf.goBackAView();
			break;
			
		case FORWARD:
			decode_pdf.goForwardAView();
			break;
			
		case ADDVIEW:
			decode_pdf.addAView(((Integer)args[0]).intValue(),(Rectangle)args[1],(Integer)args[2]);
			break;
        ...
        case CURRENTPAGE:
            ...
            if(decode_pdf==null)
                ...
                status= new Integer(currentGUI.getPageNumber());
        ...
        case PAGECOUNT:
            ...
            if(decode_pdf==null)
                ...
                status= new Integer(decode_pdf.getPageCount());
...
//			worker.start();
	...
	public String copySelectedText() {
        ...
        String returnValue = "";
        Rectangle[] selected = decode_pdf.getHighlightedAreas(commonValues.getCurrentPage());
        if(selected==null)
            return "";
...
//			int t_y2=commonValues.m_y2;
	...
	private JScrollPane updateExtractionExample(AbstractButton button, boolean xml) throws PdfException {
		...
		Rectangle[] areas= decode_pdf.getHighlightedAreas(commonValues.getCurrentPage());
	...
	public void extractSelectedText() {
		...
		final Rectangle[] selected = decode_pdf.getHighlightedAreas(commonValues.getCurrentPage());
                ...
                if(inputStream!=null || selectedFile.startsWith("http") || selectedFile.startsWith("file:") || selectedFile.startsWith("jar:")){
                    try{
                        ...
                        if(inputStream!=null)
                            ...
                            isLinearized=decode_pdf.isPDFLinearized(commonValues.getSelectedFile());

                        if(!isLinearized){

                            Point coords =null;
                            if(commonValues.getModeOfOperation()!=Values.RUNNING_APPLET)
                                coords = new Point(currentGUI.getFrame().getLocationOnScreen().x+(currentGUI.getFrame().getWidth()/2), currentGUI.getFrame().getLocationOnScreen().y+(currentGUI.getFrame().getHeight()/2));
...
//	}
			...
			if (!commonValues.isProcessing()) { //lock to stop multiple accesses
				...
				if (updatedTotal <= commonValues.getPageCount()) {
					...
					if(commonValues.isMultiTiff()){
						 ...
						 */
						if(decode_pdf.getDisplayView() == Display.FACING || decode_pdf.getDisplayView() == Display.CONTINUOUS_FACING){
							if (decode_pdf.separateCover || decode_pdf.getDisplayView() != Display.FACING) {
                                updatedTotal++;
	...
	public void gotoPage(String page) {
		...
		try{
			 ...
			 */
			if(decode_pdf.getDisplayView() == Display.FACING || decode_pdf.getDisplayView() == Display.CONTINUOUS_FACING){
				if((decode_pdf.separateCover || decode_pdf.getDisplayView() != Display.FACING) && (newPage & 1)==1 && newPage!=1){
					newPage--;
				} else if (!decode_pdf.separateCover && (newPage & 1)==0) {
                    newPage--;
                }
			}
