...
public class ScreenDisplay extends BaseDisplay implements DynamicVectorRenderer{
    ...
    public ScreenDisplay(int pageNumber, boolean addBackground, int defaultSize, ObjectStore newObjectRef) {
    ...
    public ScreenDisplay(int pageNumber, ObjectStore newObjectRef, boolean isPrinting) {
    	...
    	for(int i=0;drawPDFShapes && i<count;i++){
    		...
    		if(type>0){
    			...
    			if(type==DynamicVectorRenderer.REUSED_IMAGE){
    				...
    				imageUsed=((Integer)currentObject).intValue();
    ...
    private void renderImage(double[] afValues1, double[] afValues2,
    	...
    	if(useHiResImageForDisplay && !isType3Font && objectStoreRef.isRawImageDataSaved(key)){// && useHiResImageForDisplay){
    		...
    		int defaultX=((Integer) objectStoreRef.getRawImageDataParameter(key,ObjectStore.IMAGE_pX)).intValue();
    		int pX=(int)(defaultX*scalingToUse);

    		int defaultY=((Integer) objectStoreRef.getRawImageDataParameter(key,ObjectStore.IMAGE_pY)).intValue();
    		pY=(int)(defaultY*scalingToUse);

    		w1=((Integer) objectStoreRef.getRawImageDataParameter(key,ObjectStore.IMAGE_WIDTH)).intValue();
    		int h1=((Integer) objectStoreRef.getRawImageDataParameter(key,ObjectStore.IMAGE_HEIGHT)).intValue();

    		byte[] maskCol=(byte[]) objectStoreRef.getRawImageDataParameter(key,ObjectStore.IMAGE_MASKCOL);

    		int colorspaceID=((Integer) objectStoreRef.getRawImageDataParameter(key,ObjectStore.IMAGE_COLORSPACE)).intValue();
...
//    				if(pageRotation != 90 && pageRotation != 270 && maskCol==null){//
//
//    					//will already be turned so reset
//    					currentImageOption=PDFImageProcessing.TURN_ON_DRAW;
//    					if(imageOptions!=null)
//    						imageOptions[iCount]=currentImageOption;
//
//    				}
    			}
    			//<end-me>
    ...
    public int drawImage(int pageNumber,BufferedImage image,
        ...
        storedImageValues.put("imageOptions-"+currentItem,new Integer(optionsApplied));
    ...
    private int redrawImage(int pageNumber, GraphicsState currentGraphicsState, String name,int previousUse) {
        ...
        imageOptions.addElement(((Integer)storedImageValues.get("imageOptions-"+previousUse)).intValue());
        ...
        pageObjects.addElement(new Integer(previousUse));

        //store id so we can get as low res image
        imageID.put(name,new Integer(previousUse));
    ...
    public void drawShape(Shape currentShape,GraphicsState currentGraphicsState) {
    ...
    public void drawEmbeddedText(float[][] Trm,int fontSize,PdfGlyph embeddedGlyph,
                                 Object javaGlyph, int type,GraphicsState gs, AffineTransform at, String glyf, PdfFont currentFontData) {
     ...
     *
     */
    public ScreenDisplay(byte[] stream, Map fonts){
        ...
        try{
            ...
            int fontCount=((Integer) RenderUtils.restoreFromStream(bis)).intValue();
            ...
            int alteredFontCount=((Integer) RenderUtils.restoreFromStream(bis)).intValue();
    ...
    public byte[] serializeToByteArray(Set fontsAlreadyOnClient) throws IOException{
         ...
         * new fonts
         */
        RenderUtils.writeToStream(bos,new Integer(fontCount),"new Integer(fontCount)");
         ...
         * new fonts
         */
        RenderUtils.writeToStream(bos,new Integer(updateCount),"new Integer(existingfontCount)");
