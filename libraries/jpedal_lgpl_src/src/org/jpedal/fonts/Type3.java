...
public class Type3 extends PdfFont {
	...
	final public void createFont(PdfObject pdfObject, String fontID, boolean renderPage, ObjectStore objectStore, Map substitutedFonts) throws Exception{
		...
		readEmbeddedFont(pdfObject,objectStore, pageResources);
    ...
    private void readEmbeddedFont(PdfObject pdfObject, ObjectStore objectStore, PdfObject pageResources) {
        ...
        if(CharProcs!=null){
            ...
            T3StreamDecoder glyphDecoder=new T3StreamDecoder(currentPdfFile,hires);
            glyphDecoder.setParameters(false,true,7,0);

            glyphDecoder.setObjectValue(ValueTypes.ObjectStore, objectStore);

            //add any pageResources which may be used
            if (pageResources != null){
                currentPdfFile.checkResolved(pageResources);
                try {
                    glyphDecoder.readResources(pageResources,true);
