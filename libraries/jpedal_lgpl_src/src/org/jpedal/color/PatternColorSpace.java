...
public class PatternColorSpace extends GenericColorSpace{
	...
	private PdfPaint setupTiling(PdfObject PatternObj,float[] inputs, float[][] matrix,byte[] streamData) {
         ...
         */
        if(rawXStep==-32768 && rawYStep==-32768){

            //get the image
            BufferedImage image=glyphDisplay.getLastImageRenderer();
            ...
            if(image!=null)
                return new PdfTexturePaint(image, new Rectangle((int) (inputs[4]-image.getWidth()),0,image.getWidth(),image.getHeight()));
