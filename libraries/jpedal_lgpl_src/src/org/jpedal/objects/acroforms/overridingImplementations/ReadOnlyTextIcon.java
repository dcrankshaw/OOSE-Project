...
//			debug = true;
	...
	private void checkAndCreateimage() {
		...
		if(textChanged || rootImage==null 
			...
			rootImage = FormStream.decode(currentpdffile, fakeObj, subtype,newWidth,newHeight,0,iconRotation);
	...
	public void setText(String str){
		...
		fakeObj.setDecodedStream(fullCommandString.getBytes());
	}
...
//							strFound = true;
        ...
        if(DA==null || DA.length()==0){
        	if(fontStr.length()!=0){
        		...
        		form.setTextStreamValue(PdfDictionary.DA, fontStr.getBytes());
        	...
        	//use defined DA and remove any whitespace at front
        	fontStr = DA.trim();
        }
...
//		XObject.setFloatArray(PdfDictionary.Matrix,new float[]{1,0,0,1,0,0});
		...
		if(preFontStream.length()==0 || preFontStream.indexOf("BT")==-1){
