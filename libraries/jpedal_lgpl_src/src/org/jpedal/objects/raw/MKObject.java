...
public class MKObject extends FormObject {
	...
	public PdfObject duplicate(){
		...
		for(int jj=0;jj<textStreams.length;jj++){
			byte[] bytes=this.getTextStreamValueAsByte(textStreams[jj]);
			if(bytes!=null)
				copy.setTextStreamValue(textStreams[jj],bytes);
		}
		...
		for(int jj=0;jj<floatStreams.length;jj++){
			float[] floats=this.getFloatArray(floatStreams[jj]);
			if(floats!=null)
				copy.setFloatArray(floatStreams[jj],floats);
		}
