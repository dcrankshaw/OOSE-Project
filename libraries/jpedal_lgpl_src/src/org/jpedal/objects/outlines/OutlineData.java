...
public class OutlineData {
	...
	public int readOutlineFileMetadata(PdfObject OutlinesObj, PdfObjectReader currentPdfFile, PageLookup pageLookup) {
        ...
        if(FirstObj !=null){
			...
			int level=0;
			readOutlineLevel(root,currentPdfFile, pageLookup, FirstObj, level, false);
	 ...
	 * read a level
	 */
	private void readOutlineLevel(Element root,PdfObjectReader currentPdfFile, PageLookup pageLookup, PdfObject outlineObj, int level, boolean isClosed) {
        ...
        if (DestObj != null && DestObj.getTokenCount()>0){// && type==PdfDictionary.Goto) {
			...
			if(FirstObj!=null)
				readOutlineLevel(child,currentPdfFile, pageLookup, FirstObj, level+1, isClosed);
